...
async function run(filename: string): Promise<E.Either<RefinedCError, void>> {
    ...
    const [outcome, finalState] = specAgent(filename);
...
async function main(filename: string): Promise<boolean> {
    ...
    try {
        ...
        if (E.isRight(result)) {
            logger.info('Specification verification succeeded');
            return true;
        } else {
            logger.info('Specification verification failed', {
        ...
        logger.error('Unexpected error in specification process', { error: err });
        return false;
    }
}
